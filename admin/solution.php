...
<?php if ($UAC <= 2): ?>
    <article class="md:bg-white dark:md:bg-slate-900 md:p-6 rounded-xl w-full">
        <h4 class="mb-8 font-semibold">You can update the contents of <a href="<?= $root ?>solutions" target="_blank" class="text-blue-600 dark:text-blue-300 border-b-2 border-dashed border-blue-600 dark:border-blue-300">solutions</a> page</h4>
        <form action="modules/solution_update.php?ctid=<?= $title_id ?>&cid=<?= $desc_id ?>&ptid=<?= $ptid ?>" id="update_solution" method="POST" enctype="multipart/form-data" class="flex flex-col md:grid md:grid-cols-6 gap-6">
            <div class="col-span-2 flex flex-col gap-4">
                <div class="overflow-hidden rounded-xl w-full h-72 relative border-2 border-dashed border-blue-400 dark:border-blue-300 fileupload group">
                    <input type="file" name="solution_img" id="solution_img" accept="image/*" class="absolute left-0 h-full w-full opacity-0 cursor-pointer"
                        onchange="setupImagePreview(this, 'solution_img', true)" oninput="checkImage(this, false, false, true)">
                    <img src="<?= $updir ?>uploads/solution_side.webp?<?= time() ?>" alt="solution Side Image" class="h-full w-full object-cover">
                    <span class="editbtn">
                        <?= iPen('1.4rem') ?>
                    </span>
                </div>
                <!-- <div class="flex gap-4 h-28">
                    <div class="overflow-hidden rounded-xl w-full h-full relative border-2 border-dashed border-blue-400 dark:border-blue-300 fileupload group">
                        <input type="file" name="solution_bottoml_img" id="solution_bottoml_img" accept="image/*" class="absolute left-0 h-full w-full opacity-0 cursor-pointer"
                            onchange="setupImagePreview(this, 'solution_bottoml_img', true)" oninput="checkImage(this, false, false, true)">
                        <img src="<?= $updir ?>uploads/solution_bottoml.webp?<?= time() ?>" alt="Solution Bottom Left Image" class="h-full w-full object-cover">
                        <span class="editbtn editbtn-sm">
                            <?= iPen('1rem') ?>
                        </span>
                    </div>
                    <div class="overflow-hidden rounded-xl w-full h-full relative border-2 border-dashed border-blue-400 dark:border-blue-300 fileupload group">
                        <input type="file" name="solution_bottomr_img" id="solution_bottomr_img" accept="image/*" class="absolute left-0 h-full w-full opacity-0 cursor-pointer"
                            onchange="setupImagePreview(this, 'solution_bottomr_img', true)" oninput="checkImage(this, false, false, true)">
                        <img src="<?= $updir ?>uploads/solution_bottomr.webp?<?= time() ?>" alt="Solution Bottom Right Image" class="h-full w-full object-cover">
                        <span class="editbtn editbtn-sm">
                            <?= iPen('1rem') ?>
                        </span>
                    </div>
...
<?php if ($UAC <= 3): ?>
    <article class="flex flex-col md:flex-row gap-4 items-start justify-start border-t-2 border-dashed dark:border-slate-800 md:border-0 pt-6" id="solutionsadd">
        <section class="md:bg-white dark:md:bg-slate-900 md:p-6 rounded-xl w-full md:w-[400px]">
            <h4 class="mb-6 font-semibold">Add solutions from here</h4>
            <form action="modules/solution_add.php" method="post" enctype="multipart/form-data" class="">
                <div class="relative text-center h-52 w-full lg:w-80 rounded-2xl border-2 border-dashed border-blue-400 dark:border-blue-300 fileupload overflow-hidden">
                    <label for="solutionimg" class="font-medium select-none grid place-items-center text-blue-500 dark:text-blue-200">
                        <span class=""><?= iUpload('4rem') ?></span>
                        Drag & Drop Image Or Browse
                    </label>
                    <input type="file" name="solution_img" id="solutionimg" accept="image/*" class="absolute left-0 h-full w-full opacity-0 cursor-pointer"
                        onchange="setupImagePreview(this, 'solutionimg')" oninput="checkImage(this, true, true)" required>
                ...
                <div class="mt-6 w-full ring-1 ring-slate-400 rounded dark:bg-slate-900 dark:ring-slate-600 focus-within:ring-2 focus-within:ring-blue-600 dark:focus-within:ring-blue-300 [&:has(input:focus:not(:placeholder-shown):invalid)]:ring-red-500 group relative transition-all">
                    ...
                    <input tabindex="1" title="Solution title" class="block p-4 w-full text-slate-700 dark:text-slate-300 placeholder-transparent bg-transparent focus:outline-none group-[&:user-invalid]:ring-red-500 rounded" type="text" placeholder="" aria-label="Title" name="solution_title" id="solution_title" value="" required />
                </div>
        ...
        <section class="bg-white dark:bg-slate-900 rounded-xl w-full md:p-6 mt-10 md:mt-0">
            <form action="#solutionsadd" method="GET" class="flex gap-4 items-center justify-start w-full mb-5">
                <label for="searchtext" action="" method="GET" class="w-full cursor-text border-2 py-3.5 px-3 md:px-6 flex items-center justify-start gap-4 border-slate-200 dark:border-slate-600 rounded-lg focus-within:border-slate-400 dark:focus-within:border-slate-400 transition-colors">
                    ...
                    <input type="search" name="q" id="searchtext" tabindex="1" class="h-full focus:outline-none bg-transparent w-full"
                        ...
                        placeholder="Search solutions here..." required>
                </label>
            ...
            <div class="flex flex-wrap gap-6 place-content-start text-center mt-8">
                <?php
                if (count($projects) > 0) {
                    foreach ($projects as $in => $item) { ?>
                        <div class="w-full md:w-[180px] bg-white dark:bg-slate-800 shadow rounded-2xl overflow-hidden relative" title="<?= $item['title'] ?>">
                            <form action="modules/delete_project.php?id=<?= $item['id'] ?>" id="form_delete_project_<?= $in ?>" method="POST">
                    ...
                    <div class="w-full text-center opacity-50 flex flex-col lg:flex-row justify-center items-center gap-2 py-16 px-4">
                        <?= iSad("1.5rem") ?>
                        There are no solutions published yet.
                    </div>
                <?php } ?>
            </div>
        </section>
